...
class Shuffle:
    def __init__(*, self,
        ...
        self.input_num_partitions = input_num_partitions
        self.output_num_partitions = output_num_partitions
        self.output_writer = output_writer
        self.partitioner = partitioner
        self.object_store_writer = object_store_writer
        self.tracker = tracker
        self.streaming = streaming
    
    def shuffle_map(self, i: PartitionID) -> List[List[Union[Any, ObjectRef]]]:
        writers = [self.object_store_writer() for _ in range(self.output_num_partitions)]
        for out_i, item in self.partitioner(self.input_reader(i), self.output_num_partitions):
            writers[out_i].add(item)
        return [c.finish() for c in writers]

    def shuffle_reduce(
        self, i: PartitionID, *mapper_outputs: List[List[Union[Any, ObjectRef]]]
        ) -> OutType:
        input_objects = []
        assert len(mapper_outputs) == self.input_num_partitions
        for obj_refs in mapper_outputs:
            for obj_ref in obj_refs:
                input_objects.append(obj_ref)
        return self.output_writer(i, input_objects)
    
    def shuffle(self):
        # Do we have to make shuffle_map as remote function?
        shuffle_map_out = [self.shuffle_map(i) for i in range(self.input_num_partitions)]

        # Do we have to make shuffle_reduce as remote function?
        shuffle_reduce_out = [
            self.shuffle_reduce(
                j, *[shuffle_map_out[i][j] for i in range(self.input_num_partitions)]
            )
...
def run(
    ...
    start = time.time()
    # Instanciate actor shuffle
    shuffler = Shuffle.remote(input_reader=input_reader,
