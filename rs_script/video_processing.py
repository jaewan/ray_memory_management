...
def process_chunk(video_index, video_pathname, sink, num_frames, fps, resource, v07, start_timestamp, checkpoint_interval):
    ...
    print("DEBUGGING: ", video_pathname)
...
def process_videos(video_pathname, num_videos, output_filename, view,
    ...
    num_total_frames = int(min(v.get(cv2.CAP_PROP_FRAME_COUNT), max_frames)) # this works fine
    fps = int(v.get(cv2.CAP_PROP_FPS)) # this works fine
    ...
    for i in range(num_videos):
        ...
        print("sinks[i len sinks]", sinks[i % len(sinks)])
        print("num total frames", num_total_frames)
        print("FPS", fps)
        print("worker resource", worker_resource)
        print("v07", v07)
        print("start_timestamp", start_timestamp)
        print("checkpoint_interval", checkpoint_interval)
        process_chunk.options(resources={owner_resource: 0.001}).remote( # PROBLEM IS RIGT HERE
...
def main(args):
    num_worker_nodes = 1
    num_owner_nodes = 1 // args.max_owners_per_node
    ray.init(address="auto")
    if 1 % args.max_owners_per_node != 0:
        num_owner_nodes += 1
    num_sinks = 1 // args.max_videos_per_sink
    if 1 % args.max_videos_per_sink != 0:
        num_sinks += 1
    ...
    print("head_node_resource: ", head_node_resource)
    if args.local:
    ...
    if args.failure or args.owner_failure:
        ...
        print("args.video_path", args.video_path)
        print("args.num_videos", args.num_videos)
        print("args.output", args.output) # none 
        print("args.local", args.local) # false
        print("head_node_resource", head_node_resource)
        print("worker_resources", worker_resources)
        print("owner_resources", owner_resources)
        print("sink_resources", sink_resources)
        print("args.max_frames", args.max_frames)
        print("num_sinks", num_sinks) # 1
        print("args.v07", args.v07) # false
        print("args.checkpoint_interval", args.checkpoint_interval) # 0
        print("offset_seconds", offset_seconds) # 5
