...
class SpillRemoteManager {
 public:
  ...
  void StartSpillRemote(const NodeID &dest_id,
  ...
  void OnChunkComplete(const NodeID &dest_id, const ObjectID &obj_id, const std::function<void()> callback);
  ...
  int64_t NumPushesInFlight() const { return spill_remote_info_.size(); };
 ...
 private:
  FRIEND_TEST(TestSpillRemoteManager, TestSpillRemoteState);
  /// Tracks the state of an active object push to another node.
  struct SpillRemoteState {
    ...
    SpillRemoteState(int64_t num_chunks, std::function<void(int64_t)> chunk_send_fn)
  ...
  absl::flat_hash_map<SpillRemoteID, std::unique_ptr<SpillRemoteState>> spill_remote_info_;
};
